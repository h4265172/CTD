...
class CTD():
    ...
    def _temporal_params(self, Y=None, batch=True):
        ...
        with torch.no_grad():
            ...
            past = torch.empty((T - self.AR_lags.max(), 
                                (self.AR_lags.shape[0] * Y.shape[1] + 1)), device=self.device).float()
            past[:,0] = 1.
    ...
    def _step_XY(self, batch=True):
        ...
        loss = temporal_reg + main + self.lambda_all * (Y.norm().pow(2)/self.N + X.norm().pow(2)/self.T) #+ cluster_reg
        loss.backward()
    ...
    def pretrain(self, batch=True):
        ...
        print("## Pretraining done after loop {} | pretrain loss {}".format(self.pretrain_loop_count, loss))
